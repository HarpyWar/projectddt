...
const char *libmpq__version(void) {
	...
	return "libMPQ Project DDT Internal Version"; /* DDT edit */
}
...
/* DDT edit */ /* Feed this a file name WITHOUT PATH to get the right file crypt key */
int32_t libmpq__key_from_name(const char *filename) {

	return libmpq__hash_string(filename, 0x300);
}
...
/* DDT edit */ /*added file_key input requirement */
int32_t libmpq__file_read(mpq_archive_s *mpq_archive, uint32_t file_number, uint8_t *out_buf, libmpq__off_t out_size, libmpq__off_t *transferred, uint32_t file_key) {
	...
	if ((result = libmpq__block_open_offset(mpq_archive, file_number, file_key)) < 0) {
...
/* DDT edit */ /*added file_key input requirement */
int32_t libmpq__block_open_offset(mpq_archive_s *mpq_archive, uint32_t file_number, uint32_t file_key) {
	...
	/* DDT edit */ /*Just set the file key, no need for trial and error */
	mpq_archive->mpq_file[file_number]->seed = file_key;
